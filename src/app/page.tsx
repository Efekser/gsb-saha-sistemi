...
export default function Home() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-4 md:py-12 px-4 font-roboto antialiased overflow-x-hidden touch-manipulation min-h-screen flex flex-col">
      <style jsx global>{`
      ...
      <div className="flex justify-start mb-6 h-12 md:h-20">
        <div className="opacity-0 pointer-events-none">GSB</div>
      </div>

      {!islemTuru ? (
        <div className="flex-1 flex flex-col items-center justify-center space-y-10 animate-in fade-in duration-700 px-2">
           <div className="text-center">
             <h1 className="text-4xl md:text-6xl font-black text-[#002B67] uppercase tracking-tighter leading-tight">GSB MANAVGAT</h1>
             <div className="h-1.5 w-20 bg-[#E30A17] mx-auto rounded-full mt-3"></div>
           </div>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-2xl px-4">
             <button onClick={() => setIslemTuru("al")} className="group p-8 md:p-12 bg-[#002B67] text-white rounded-[2.5rem] shadow-2xl hover:scale-105 active:scale-95 transition-all flex flex-col items-center border-b-8 border-blue-950">
               <span className="text-5xl mb-4 group-hover:animate-bounce">⚽</span>
               <span className="font-black text-xl tracking-widest uppercase">Randevu Al</span>
             </button>
             <button onClick={() => setIslemTuru("sorgula")} className="group p-8 md:p-12 bg-white text-[#002B67] border-4 border-[#002B67] rounded-[2.5rem] shadow-2xl hover:scale-105 active:scale-95 transition-all flex flex-col items-center border-b-8 border-slate-200">
               <span className="text-5xl mb-4 group-hover:animate-[spin_3s_linear_infinite]">🔍</span>
               <span className="font-black text-xl tracking-widest uppercase">Sorgulama</span>
             </button>
           </div>
        </div>
      ) : islemTuru === "sorgula" ? (
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-500 max-w-2xl mx-auto w-full px-1">
           <button onClick={anasayfayaDon} className="mb-6 font-black text-[#002B67] flex items-center gap-2 hover:translate-x-[-4px] transition-all text-sm">← GERİ DÖN</button>
           <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-50 p-6 md:p-10">
             ...
             <div className="space-y-4 mb-10">
               <div className="space-y-2 text-center">
                 <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Kayıtlı E-Posta</label>
                 <input 
                   ref={firstInputRef}
                   type="email"
                   ...
                   className="w-full h-16 px-6 rounded-2xl border-2 border-slate-100 bg-slate-50 focus:bg-white focus:border-[#002B67] outline-none font-bold text-lg text-center shadow-inner"
                 />
               </div>
               <button onClick={handleSorgula} className="w-full py-5 rounded-2xl font-black bg-[#002B67] text-white uppercase shadow-xl transition-all tracking-widest">SORGULA</button>
             </div>
               ...
               <div className="space-y-6 animate-in zoom-in duration-500">
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 px-2">
                   {bulunanRandevu.durum === "Beklemede" && iptalButonuAktif(bulunanRandevu.tarih) && (
                     <button className="py-5 bg-green-600 text-white rounded-2xl font-black text-sm uppercase shadow-lg">KESİNLEŞTİR</button>
                   )}
                   {iptalButonuAktif(bulunanRandevu.tarih) ? (
                     <button className="py-5 bg-white text-red-600 border-2 border-red-600 rounded-2xl font-black text-sm uppercase shadow-sm">İPTAL ET</button>
                   ) : (
                     <div className="col-span-full p-5 bg-red-50 rounded-xl text-red-500 text-[10px] font-bold text-center border border-red-100 uppercase tracking-tighter">
                       İptal süresi (24 saat) dolmuştur.
                     </div>
                   )}
                 </div>
               </div>
        ...
        <div className="flex-1 animate-in fade-in duration-500 px-1">
          <button onClick={anasayfayaDon} className="mb-6 font-black text-[#002B67] flex items-center gap-2 text-sm uppercase">← Ana Ekran</button>
          
          <div className="flex justify-between mb-10 relative max-w-2xl mx-auto px-4">
            <div className="absolute top-1/2 left-0 w-full h-0.5 bg-slate-100 -translate-y-1/2 -z-10" />
            {adimlar.map((i) => (
              <div key={i.n} className="flex flex-col items-center gap-2 relative bg-white px-1">
                <div className={`w-8 h-8 md:w-11 md:h-11 rounded-full flex items-center justify-center font-black text-xs transition-all border-2 ${adim >= i.n ? "bg-[#002B67] border-[#002B67] text-white" : "bg-white text-slate-300 border-slate-100"}`}>{i.n}</div>
                <span className={`text-[6px] md:text-[9px] font-bold uppercase tracking-tighter ${adim >= i.n ? "text-[#002B67]" : "text-slate-200"}`}>{i.b}</span>
              </div>
            ))}
          </div>

          <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-50 p-5 md:p-12 min-h-[500px] flex flex-col justify-center relative overflow-hidden">
            {adim === 1 && (
            ...
            {adim === 2 && (
              <div className="space-y-8 animate-in fade-in text-center">
                ...
                <div className="flex flex-col md:flex-row gap-4 max-w-xs mx-auto">
                    <button onClick={() => setAdim(1)} className="flex-1 py-5 rounded-2xl font-black border-2 border-[#002B67] text-[#002B67] uppercase transition-all hover:bg-slate-50 text-xs">Geri Dön</button>
                    <button disabled={!seciliSaha || !seciliSaat} onClick={() => setAdim(3)} className="flex-[2] py-5 rounded-2xl font-black bg-[#002B67] text-white uppercase shadow-lg block disabled:opacity-50 text-xs">Devam Et</button>
                </div>
              </div>
            )}

            {adim === 3 && (
              <div className="space-y-5 max-w-md mx-auto animate-in fade-in w-full px-2 text-left">
                <h2 className="text-2xl font-black text-[#002B67] text-center uppercase mb-4 tracking-tighter">Kayıt Bilgileri</h2>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-2 tracking-widest">Ad</label>
                    <input ref={firstInputRef} placeholder="Adınız" value={formData.ad} onChange={(e) => handleInputChange("ad", e.target.value)} className="w-full h-12 px-4 rounded-xl border-2 border-slate-100 bg-white focus:border-[#E30A17] outline-none font-bold text-sm uppercase shadow-sm" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-2 tracking-widest">Soyad</label>
                    <input placeholder="Soyadınız" value={formData.soy_ad} onChange={(e) => handleInputChange("soy_ad", e.target.value)} className="w-full h-12 px-4 rounded-xl border-2 border-slate-100 bg-white focus:border-[#E30A17] outline-none font-bold text-sm uppercase shadow-sm" />
                  </div>
                </div>
                <div className="space-y-4 pt-2">
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-2 tracking-widest">Telefon</label>
                    <input type="tel" placeholder="05XX XXX XX XX" value={formData.telefon} onChange={(e) => handleInputChange("telefon", e.target.value)} className="w-full h-12 px-4 rounded-xl border-2 border-slate-100 bg-white focus:border-[#E30A17] outline-none font-bold text-sm shadow-sm" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-bold text-slate-400 uppercase ml-2 tracking-widest">E-Posta</label>
                    <input type="email" placeholder="örnek@mail.com" value={formData.eposta} onChange={(e) => handleInputChange("eposta", e.target.value)} className="w-full h-12 px-4 rounded-xl border-2 border-slate-100 bg-white focus:border-[#E30A17] outline-none font-medium text-sm shadow-sm" />
                  </div>
                </div>
                <div className="flex flex-col md:flex-row gap-4 pt-4">
                    <button onClick={() => setAdim(2)} className="flex-1 py-5 rounded-2xl font-black border-2 border-[#002B67] text-[#002B67] uppercase transition-all text-xs">Geri Dön</button>
                    <button disabled={!formGecerli} onClick={() => setAdim(4)} className="flex-[2] py-5 rounded-2xl font-black bg-[#002B67] text-white uppercase shadow-xl disabled:opacity-50 text-xs">Kod Gönder</button>
                </div>
              </div>
            )}

            {adim === 4 && (
              <div className="space-y-8 max-w-md mx-auto animate-in zoom-in text-center w-full px-2">
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    <button onClick={() => setAdim(3)} className="flex-1 py-5 rounded-2xl font-black border-2 border-[#002B67] text-[#002B67] uppercase text-xs">Geri</button>
                    <button disabled={onayKodu.length !== 6} onClick={() => setAdim(5)} className="flex-[2] py-5 rounded-2xl font-black bg-[#002B67] text-white uppercase shadow-xl disabled:opacity-50 text-xs">İleri</button>
                </div>
              </div>
            )}

            {adim === 5 && (
              <div className="animate-in fade-in w-full px-1 text-left flex flex-col min-h-[400px]">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 mb-6 overflow-y-auto pr-1 scrollbar-thin ios-scroll max-h-[350px]">
                  {katilimcilar.slice(0, katilimciSayisi).map((k, index) => (
                    <input 
                      ...
                      placeholder={`${index + 1}. Katılımcı Ad Soyad`} 
                      className="h-12 px-4 rounded-xl border-2 border-slate-100 bg-white focus:border-[#E30A17] outline-none font-bold uppercase text-[11px] shadow-sm" 
                    />
                  ))}
                </div>
                <div className="flex flex-col md:flex-row gap-4 mt-auto">
                    <button onClick={() => setAdim(3)} className="flex-1 py-5 rounded-2xl font-black border-2 border-[#002B67] text-[#002B67] uppercase text-xs">Geri</button>
                    <button disabled={!takimGecerli} onClick={() => setAdim(6)} className="flex-[2] py-5 rounded-2xl font-black bg-[#002B67] text-white shadow-xl disabled:opacity-50 text-xs uppercase">Randevu Al</button>
                </div>
              </div>
            )}

            {adim === 6 && (
              <div className="text-center py-6 animate-in zoom-in w-full px-2">
                ...
                <div className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 text-[10px] font-bold text-slate-400 space-y-4 uppercase tracking-widest text-left relative overflow-hidden shadow-inner mb-8">
                  <div className="absolute top-0 left-0 w-2 h-full bg-[#E30A17]" />
                  <div className="flex justify-between border-b border-white pb-3 px-2"><span>Sorumlu</span><span className="text-[#002B67] uppercase font-black">{formData.ad} {formData.soy_ad}</span></div>
                  ...
                  <div className="flex justify-between border-b border-white pb-3 px-2"><span>Saha / Saat</span><span className="text-[#002B67] uppercase font-black">{seciliSaha} / {seciliSaat}</span></div>
                  <div className="flex justify-between px-2"><span>Tarih</span><span className="text-[#002B67] font-black uppercase">{seciliGun} {ayIsmi}</span></div>
                </div>
