...
export default function Home() {
  ...
  return (
    <div className={`w-full transition-colors duration-300 ${isDarkMode ? "bg-[#0f172a] text-white" : "bg-white text-slate-900"} font-roboto antialiased overflow-x-hidden touch-manipulation min-h-screen flex flex-col`}>
      <style jsx global>{`
        ...
        .scrollbar-thin::-webkit-scrollbar-thumb { background: ${isDarkMode ? "#334155" : "#cbd5e1"}; border-radius: 10px; }
      `}</style>
      ...
      {!islemTuru ? (
        <div className="flex-1 flex flex-col items-center justify-center space-y-10 animate-in fade-in duration-700 px-4">
           <div className="text-center">
             <h1 className={`text-4xl md:text-6xl font-black ${isDarkMode ? "text-white" : "text-[#002B67]"} uppercase tracking-tighter leading-tight`}>GSB MANAVGAT</h1>
             <div className="h-1.5 w-24 bg-[#E30A17] mx-auto rounded-full mt-2"></div>
           </div>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl">
             <button onClick={() => islemBaslat("al")} className="group p-8 md:p-10 bg-[#002B67] text-white rounded-[2.5rem] shadow-2xl hover:scale-105 active:scale-95 transition-all flex flex-col items-center border-b-8 border-blue-950">
             ...
             <button onClick={() => islemBaslat("sorgula")} className={`group p-8 md:p-10 ${isDarkMode ? "bg-slate-800 text-white border-slate-700" : "bg-white text-[#002B67] border-[#002B67]"} border-4 rounded-[2.5rem] shadow-2xl hover:scale-105 active:scale-95 transition-all flex flex-col items-center border-b-8 ${isDarkMode ? "border-slate-900" : "border-slate-200"}`}>
               <span className="text-4xl mb-4 group-hover:animate-[spin_3s_linear_infinite]">🔍</span>
           ...
           <div className={`${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white border-slate-50"} rounded-[2.5rem] shadow-2xl border p-6 md:p-10`}>
             ...
             {bulunanRandevu && (
               <div className="space-y-6 animate-in zoom-in duration-500">
                 <div className={`p-8 ${isDarkMode ? "bg-slate-900" : "bg-slate-50"} rounded-[2rem] border ${isDarkMode ? "border-slate-700" : "border-slate-100"} relative shadow-inner`}>
                   ...
                   <div className="grid grid-cols-1 gap-4 uppercase text-[11px] font-bold text-slate-400 tracking-widest">
                      <div className={`flex justify-between border-b ${isDarkMode ? "border-slate-800" : "border-white"} pb-3`}><span>SORUMLU</span><span className={isDarkMode ? "text-white" : "text-[#002B67]"}>{bulunanRandevu.ad}</span></div>
                      <div className={`flex justify-between border-b ${isDarkMode ? "border-slate-800" : "border-white"} pb-3`}><span>SAHA</span><span className={isDarkMode ? "text-white" : "text-[#002B67]"}>{bulunanRandevu.saha}</span></div>
                      <div className={`flex justify-between border-b ${isDarkMode ? "border-slate-800" : "border-white"} pb-3`}><span>TARİH / SAAT</span><span className={isDarkMode ? "text-white" : "text-[#002B67]"}>{bulunanRandevu.tarih} {ayIsmi.toUpperCase()} / {bulunanRandevu.saat}</span></div>
                      <div className="flex justify-between"><span>DURUM</span><span className={bulunanRandevu.durum === "Kesinleşti" ? "text-green-500 font-black" : "text-orange-500 font-black"}>{bulunanRandevu.durum.toUpperCase()}</span></div>
                   </div>
                 </div>
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   ...
                   {iptalButonuAktif(bulunanRandevu.tarih) ? (
                     <button className={`py-5 ${isDarkMode ? "bg-slate-700 text-red-400" : "bg-white text-red-600"} border-2 border-red-600 rounded-2xl font-black text-sm uppercase shadow-sm`}>İPTAL ET</button>
                   ) : (
          ...
          <div className={`${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white border-slate-50"} rounded-[2.5rem] shadow-2xl border p-6 md:p-12 min-h-[500px] flex flex-col justify-center relative overflow-hidden text-center mx-2`}>
            ...
            {adim === 2 && (
              <div className="space-y-8 animate-in fade-in text-center">
                <h2 className={`text-2xl font-black ${isDarkMode ? "text-white" : "text-[#002B67]"} uppercase tracking-tighter`}>Saha ve Saat</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[ {id: "Voleybol Sahası", e: "🏐"}, {id: "Basketbol Sahası", e: "🏀"} ].map((s) => (
                    <button key={s.id} onClick={() => setSeciliSaha(s.id)} className={`p-6 rounded-2xl border-4 font-black transition-all flex items-center gap-4 ${seciliSaha === s.id ? (isDarkMode ? "border-white bg-slate-700 text-white" : "border-[#002B67] bg-white text-[#002B67] shadow-xl") : (isDarkMode ? "border-slate-900 bg-slate-900 text-slate-500" : "border-slate-50 bg-slate-50 text-slate-400")}`}>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 max-h-[200px] overflow-y-auto p-2 scrollbar-thin ios-scroll">
                  {tumSaatler.map((s) => {
                    ...
                    return (
                      <button key={s} disabled={gecmis} onClick={() => setSeciliSaat(s)} className={`py-4 rounded-xl border-2 font-black text-[10px] md:text-xs transition-all ${gecmis ? "opacity-20 pointer-events-none" : seciliSaat === s ? "bg-[#002B67] text-white border-[#002B67] shadow-md" : isDarkMode ? "bg-slate-900 border-slate-700 text-slate-400" : "bg-white border-slate-100 text-slate-600"}`}>{s}</button>
                    );
                  })}
                </div>
            ...
            {adim === 3 && (
              <div className="space-y-5 max-w-md mx-auto animate-in fade-in duration-500 w-full px-2 text-left">
                <h2 className={`text-2xl font-black ${isDarkMode ? "text-white" : "text-[#002B67]"} text-center uppercase mb-4 tracking-tighter`}>Kayıt Bilgileri</h2>
            ...
            {adim === 4 && (
              <div className="space-y-8 max-w-md mx-auto animate-in zoom-in duration-500 text-center w-full px-2">
                <h2 className={`text-2xl font-black ${isDarkMode ? "text-white" : "text-[#002B67]"} uppercase tracking-tighter`}>Doğrulama</h2>
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                  <span className={`normal-case font-black ${isDarkMode ? "text-white" : "text-[#002B67]"}`}>{formData.eposta}</span> ADRESİNE GELEN KODU GİRİNİZ
                </p>
            ...
            {adim === 5 && (
              <div className="animate-in fade-in duration-500 w-full px-2 text-left flex flex-col min-h-[400px]">
                <h2 className={`text-2xl font-black ${isDarkMode ? "text-white" : "text-[#002B67]"} text-center uppercase mb-6 tracking-tighter`}>Takım Listesi ({katilimciSayisi} Kişi)</h2>
            ...
            {adim === 6 && (
              <div className="text-center py-6 animate-in zoom-in duration-700 w-full px-2">
                <div className={`w-20 h-20 ${isDarkMode ? "bg-slate-900" : "bg-blue-50"} text-[#002B67] rounded-full flex items-center justify-center mx-auto mb-8 border-2 ${isDarkMode ? "border-slate-700" : "border-blue-100"} shadow-inner`}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke={isDarkMode ? "white" : "currentColor"} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
                <h2 className={`text-3xl font-black ${isDarkMode ? "text-white" : "text-[#002B67]"} uppercase mb-8 tracking-tighter`}>İşlem Tamam!</h2>
                <div className={`p-8 ${isDarkMode ? "bg-slate-900 border-slate-700" : "bg-slate-50 border-slate-100"} rounded-[2.5rem] border text-[10px] font-bold text-slate-400 space-y-4 uppercase tracking-widest text-left relative overflow-hidden shadow-inner mb-8`}>
                  <div className="absolute top-0 left-0 w-2 h-full bg-[#E30A17]" />
                  <div className={`flex justify-between border-b ${isDarkMode ? "border-slate-800" : "border-white"} pb-3 px-2`}><span>Sorumlu</span><span className={`${isDarkMode ? "text-white" : "text-[#002B67]"} uppercase font-black`}>{formData.ad} {formData.soy_ad}</span></div>
                  <div className={`flex justify-between border-b ${isDarkMode ? "border-slate-800" : "border-white"} pb-3 px-2`}><span>E-Posta</span><span className={`${isDarkMode ? "text-white" : "text-[#002B67]"} normal-case font-medium italic`}>{formData.eposta}</span></div>
                  <div className={`flex justify-between border-b ${isDarkMode ? "border-slate-800" : "border-white"} pb-3 px-2`}><span>Saha / Saat</span><span className={`${isDarkMode ? "text-white" : "text-[#002B67]"} uppercase font-black`}>{seciliSaha} / {seciliSaat}</span></div>
                  <div className="flex justify-between px-2"><span>Tarih</span><span className={`${isDarkMode ? "text-white" : "text-[#002B67]"} font-black uppercase`}>{seciliGun} {ayIsmi}</span></div>
                </div>
                <button onClick={anasayfayaDon} className="px-12 py-5 bg-[#002B67] text-white rounded-full font-black uppercase shadow-2xl hover:bg-[#E30A17] transition-all tracking-[0.2em] text-xs active:scale-95">ANASAYFAYA DÖN</button>
              </div>
            )}
          </div>
        </div>
